...
void OnTick()
  {
      ...
      if (CopyRates(Symbol(), EA_Timeframe, 0, barsCalculatedForSuperTrend, priceRates) <= 0) return ;
      
      ArraySetAsSeries(priceRates, true);
      
      if (PreviousTimeSaved != priceRates[0].time)
      {
         if (!CopyBuffer(HandlerSuperTrend, 2, 0, barsCalculatedForSuperTrend, superTrendBufferTrendIndicator)) return ;
         if (!CopyBuffer(HandlerStochasticRSI, 0, 0, barsCalculatedForSuperTrend, stochasticRSIBufferIndicator)) return ;
         if (!CopyBuffer(HandlerTradeDynamicIndex, 1, 0, barsCalculatedForTradeDynamicIndex, tradeDynamicIndexBufferIndicatorColorSignal)) return ;
         if (!CopyBuffer(HandlerAverageTrueRange, 0, 0, barsCalculatedForAverageTrueRange, averageTrueRangeStop)) return ;
